...
public class WorldGenTreeKaedeBig extends WorldGenAbstractTree {
	...
	List<WorldGenTreeKaedeBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTreeKaedeBig(boolean notify) {
		super(notify);
		this.leave = ASDecoModWoods.KAEDE_leaf.getDefaultState();
	}

	public WorldGenTreeKaedeBig(boolean notify, IBlockState state) {
		super(notify);
	  this.leave = state;
	}
	...
	public Block getLog() {
	  return ASDecoModWoods.KAEDE_log;
	}

	public Block getSapling() {
	  return ASDecoModWoods.KAEDE_nae;
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTreeKaedeBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTreeKaedeBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTreeKaedeBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeaves() {
		...
		for (WorldGenTreeKaedeBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
