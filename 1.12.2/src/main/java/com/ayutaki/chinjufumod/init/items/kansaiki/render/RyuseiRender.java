...
public abstract class RyuseiRender<T extends Entity> extends Render<T> {

	ModelRyusei model;
	float scale = 1;
	int modifier = 0;

	protected RyuseiRender(RenderManager renderManager, ModelRyusei model, float scale, int rotationModifier) {
		this(renderManager, model, rotationModifier);
		this.scale = scale;
	}

	protected RyuseiRender(RenderManager renderManager, ModelRyusei model, int rotationModifier) {
		super(renderManager);
		this.model = model;
