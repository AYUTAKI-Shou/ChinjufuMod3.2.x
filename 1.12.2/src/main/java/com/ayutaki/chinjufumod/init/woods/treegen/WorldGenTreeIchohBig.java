...
public class WorldGenTreeIchohBig extends WorldGenAbstractTree {
	...
	List<WorldGenTreeIchohBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTreeIchohBig(boolean notify) {
		super(notify);
		this.leave = ASDecoModWoods.ICHOH_leaf.getDefaultState();
	}

	public WorldGenTreeIchohBig(boolean notify, IBlockState state) {
		super(notify);
	  this.leave = state;
	}
	...
	public Block getLog() {
	  return ASDecoModWoods.ICHOH_log;
	}

	public Block getSapling() {
	  return ASDecoModWoods.ICHOH_nae;
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTreeIchohBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTreeIchohBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTreeIchohBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeaves() {
		...
		for (WorldGenTreeIchohBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
