...
public class BlockSakeShin_Bot_4 extends BlockFacingSapo {

	public BlockSakeShin_Bot_4() {
		super(Material.WOOD);
		...
		if (item != TTimeItems.Item_DRINKGLASS) {
			...
			if (itemstack.isEmpty()) {
				((EntityPlayer) playerIn).inventory
						.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.SHINSAKEGLA_1)));
			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.SHINSAKEGLA_1)))) {
				playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.SHINSAKEGLA_1)), false);
			}

			return worldIn.setBlockState(pos, ASDecoModHakkou.SHINSAKEBOT_kara
