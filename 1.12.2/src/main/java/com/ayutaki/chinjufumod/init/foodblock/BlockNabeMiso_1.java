...
public class BlockNabeMiso_1 extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (item != TTimeItems.Item_SHIKKI) {
			return true;
		}

		else if (item == TTimeItems.Item_SHIKKI) {
			...
			if (itemstack.isEmpty()) {
				...
				.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.MISOSOUP_1)));

			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.MISOSOUP_1)))) {
				playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.MISOSOUP_1)), false);
			}

			return  worldIn.setBlockState(pos, ChinjufuModFoodBlocks.NABEMISO_2
	...
	public void addInformation(ItemStack stack, @Nullable World playerIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.tile.block_food_nabemiso_1.name", meta));
	}
}
