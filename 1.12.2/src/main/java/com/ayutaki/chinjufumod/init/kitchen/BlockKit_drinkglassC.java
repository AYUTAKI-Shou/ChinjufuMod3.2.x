...
public class BlockKit_drinkglassC extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side,
		...
		if (itemstack.isEmpty()) {
			((EntityPlayer) playerIn).inventory.addItemStackToInventory(new ItemStack(TTimeItems.Item_DRINKGLASS));
			worldIn.playSound(null, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.PLAYERS, 0.2F, 2.0F);

			return worldIn.setBlockState(pos, ASDecoModKitchen.KIT_DRIGLASSB.getDefaultState()
			...
			return worldIn.setBlockState(pos, ASDecoModKitchen.KIT_DRIGLASSD
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		List<ItemStack> stack = new ArrayList<ItemStack>();
	        ...
	        stack.add(new ItemStack(TTimeItems.Item_DRINKGLASS, 7, this.damageDropped(state)));
