...
public abstract class TenzanRender<T extends Entity> extends Render<T> {

	ModelTenzan model;
	float scale = 1;
	int modifier = 0;

	protected TenzanRender(RenderManager renderManager, ModelTenzan model, float scale, int rotationModifier) {
		this(renderManager, model, rotationModifier);
		this.scale = scale;
	}

	protected TenzanRender(RenderManager renderManager, ModelTenzan model, int rotationModifier) {
		super(renderManager);
		this.model = model;
