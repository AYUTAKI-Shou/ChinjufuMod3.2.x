...
public class BlockZundou_coldmilk_1 extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (item != TTimeItems.NYUSAN) {
			return true;
		}

		else if (item == TTimeItems.NYUSAN) {

		   	itemstack.shrink(1);
			worldIn.playSound(null, pos, SoundsHandler.WATER_SPLASH, SoundCategory.BLOCKS, 0.5F, 1.2F);

			if (itemstack.isEmpty()) {
				((EntityPlayer) playerIn).inventory.addItemStackToInventory(new ItemStack(Items.GLASS_BOTTLE));
			...
			return worldIn.setBlockState(pos, ASDecoModHakkou.NABE_coldmilk_2
