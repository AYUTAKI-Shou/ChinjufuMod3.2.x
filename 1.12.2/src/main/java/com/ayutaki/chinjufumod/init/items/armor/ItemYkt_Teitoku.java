...
public class ItemYkt_Teitoku extends ItemArmor {
	...
	public ItemYkt_Teitoku(int renderIndexIn, EntityEquipmentSlot equipmentSlotIn) {
		super(ChinjufuModArmorMaterial.TTOKUYKT, renderIndexIn, equipmentSlotIn);
	...
	public void addInformation(ItemStack stack, @Nullable World playerIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		if(this == ChinjufuModArmor.TTOKUYKT_CHESTPLATE) {
			tooltip.add(I18n.format("tips.item.item_ykt.name", meta));
		}

		if(this == ChinjufuModArmor.TTOKUYKT_LEGGINGS) {
			tooltip.add(I18n.format("tips.item.item_ykt.name", meta));
		}
	}
	...
	public boolean getIsRepairable(ItemStack toRepair, ItemStack repair) {
		...
		if(this == ChinjufuModArmor.YKTO_BOOTS) {
			int k;
