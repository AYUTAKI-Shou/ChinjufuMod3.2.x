...
public class ItemRyusei extends ExtendedItem {

	protected final MaterialDefinition materialDefinition;
	protected final boolean unbreakable;

	public ItemRyusei(ToolMaterial material) {
		this(material, false);
	}

	public ItemRyusei(ToolMaterial material, boolean unbreakable) {
		super();
	 ...
	 * {@link EntityRyusei#getImpactDamage}
	 */
	@Override
	public float getHitDamage() {

		return Math.max(1f, (9.0f));
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand hand) {
		...
		if (hand == EnumHand.MAIN_HAND) {
			...
			if (playerIn.capabilities.isCreativeMode || KansaikiHelpers.consumeInventoryItem(playerIn, this)) {
				...
				if (!worldIn.isRemote) {
					EntityRyusei kansaiki = new EntityRyusei(worldIn, playerIn, throwStack);
