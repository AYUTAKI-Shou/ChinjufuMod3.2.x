...
public class WorldGenTreeKaede extends WorldGenAbstractTree {

	private IBlockState blockStateWood = ASDecoModWoods.KAEDE_log.getDefaultState();
	private IBlockState blockStateLeaves =
			ASDecoModWoods.KAEDE_leaf.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));

	/* 生成される木の高さの最小値 デフォルト4 */
	private final int minTreeHeight = 4;

	public WorldGenTreeKaede(boolean parShouldNotify) {
	...
	protected boolean canGrowInto(Block blockType) {
		...
		return material == Material.AIR || material == Material.LEAVES || blockType == Blocks.GRASS || blockType == Blocks
				...
				.VINE|| blockType == ASDecoModWoods.FALL_LEAF;
	}
}
