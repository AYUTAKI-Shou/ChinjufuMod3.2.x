...
public class BlockZundou_oyu extends BlockFacingSapo {
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
		...
		if (world.getBlockState(new BlockPos(i, j - 1, k)).getBlock() != Blocks.LIT_FURNACE && world
			...
			world.setBlockState(pos,ChinjufuModFoodBlocks.ZUNDOU_MIZU
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (item != TTimeItems.PASTA_nama) {
			return true;
		}

		else if (item == TTimeItems.PASTA_nama) {

		   	itemstack.shrink(1);
			worldIn.playSound(null, pos, SoundsHandler.WATER_SPLASH, SoundCategory.BLOCKS, 0.5F, 1.2F);
			return worldIn.setBlockState(pos, ChinjufuModFoodBlocks.PASTANABE_full
					.getDefaultState().withProperty(FACING, state.getValue(FACING)));
		}
		return true;
	}
