...
public class BlockKanki_spru_1 extends BlockFacingSapo {
	...
	public BlockKanki_spru_1(Material material, String unlocalizedName) {
		super(material);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (playerIn.isSneaking()) {
			...
			return worldIn.setBlockState(pos, ASDecoModRanma.KANKI_spru.getDefaultState().withProperty(FACING, state.getValue(FACING)));
		}
		return false;
	}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
			...
			boolean left = left_block.getBlock() instanceof BlockKanki_spru_1 && left_block.getValue(FACING).equals(facing);
			boolean right = right_block.getBlock() instanceof BlockKanki_spru_1 && right_block.getValue(FACING).equals(facing);
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		...
		return new ItemStack(ASDecoModRanma.KANKI_spru).getItem();
	}

	@Override
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) {

		return new ItemStack(ASDecoModRanma.KANKI_spru);
	}
