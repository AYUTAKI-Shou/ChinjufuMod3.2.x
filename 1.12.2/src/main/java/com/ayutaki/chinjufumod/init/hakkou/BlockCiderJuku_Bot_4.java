...
public class BlockCiderJuku_Bot_4 extends BlockFacingSapo {

	public BlockCiderJuku_Bot_4() {
		super(Material.WOOD);
		...
		if (item != TTimeItems.Item_DRINKGLASS) {
			...
			if (itemstack.isEmpty()) {
				((EntityPlayer) playerIn).inventory
						.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.JUKUCIDERGLA_1)));
			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.JUKUCIDERGLA_1)))) {
				playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.JUKUCIDERGLA_1)), false);
			}

			return worldIn.setBlockState(pos, ASDecoModHakkou.JUKUCIDERBOT_kara
