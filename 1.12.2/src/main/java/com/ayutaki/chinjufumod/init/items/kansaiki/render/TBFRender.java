...
public abstract class TBFRender<T extends Entity> extends Render<T> {

	ModelTBF model;
	float scale = 1;
	int modifier = 0;

	protected TBFRender(RenderManager renderManager, ModelTBF model, float scale, int rotationModifier) {
		this(renderManager, model, rotationModifier);
		this.scale = scale;
	}

	protected TBFRender(RenderManager renderManager, ModelTBF model, int rotationModifier) {
		super(renderManager);
		this.model = model;
