...
public abstract class Type97Render<T extends Entity> extends Render<T> {

	ModelType97 model;
	float scale = 1;
	int modifier = 0;

	protected Type97Render(RenderManager renderManager, ModelType97 model, float scale, int rotationModifier) {
		this(renderManager, model, rotationModifier);
		this.scale = scale;
	}

	protected Type97Render(RenderManager renderManager, ModelType97 model, int rotationModifier) {
		super(renderManager);
		this.model = model;
