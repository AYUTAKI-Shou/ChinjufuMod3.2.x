...
public class BlockGrape_1 extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (item != Items.DYE) { return true; }
			...
			/*特定のアイテムを1個消費*/
			itemstack.shrink(1);
			return  worldIn.setBlockState(pos,TTimePlants.BUDOUNOKI_4.getDefaultState()
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random random) {
		...
		if (world.getBlockState(new BlockPos(i, j - 1, k)).getBlock() == Blocks.DIRT) {
			...
			if (world.getLightFromNeighbors(pos.up()) >= 9) {
				...
				world.setBlockState(new BlockPos(i, j, k), TTimePlants.BUDOUNOKI_2.getDefaultState()
						.withProperty(FACING, state.getValue(FACING)));
			}
			world.scheduleUpdate(new BlockPos(i, j, k), this, this.tickRate(world));
		}
