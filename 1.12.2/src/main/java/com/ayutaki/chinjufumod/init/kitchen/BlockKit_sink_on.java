...
public class BlockKit_sink_on extends BlockFacingSapo {

	public BlockKit_sink_on()  {
		super(Material.WOOD);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side,
		...
		if (itemstack.isEmpty()) {
			worldIn.playSound(null, pos, SoundsHandler.WATER_STOP, SoundCategory.BLOCKS, 1.0F, 1.0F);
			return worldIn.setBlockState(pos, ASDecoModKitchen.KIT_SINK1.getDefaultState()
		...
		/*水を汲む*/
				...
				/*指定のアイテムを1個消費*/
			itemstack.shrink(1);
			worldIn.playSound(null, pos, SoundEvents.ITEM_BUCKET_FILL, SoundCategory.PLAYERS, 0.7F, 1.0F);

				if (itemstack.isEmpty()) {
					((EntityPlayer) playerIn)
					.inventory.addItemStackToInventory(new ItemStack(Items.WATER_BUCKET));
				}
		...
		/*大釜(Cauldron)から引用*/
			...
			worldIn.playSound(null, pos, SoundEvents.ITEM_BUCKET_FILL, SoundCategory.PLAYERS, 0.7F, 1.0F);
				...
				if (itemstack.isEmpty()) {
					((EntityPlayer) playerIn)
					.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.KETTLE_full)));
				}
			...
			worldIn.playSound(null, pos, SoundEvents.ITEM_BUCKET_FILL, SoundCategory.PLAYERS, 0.7F, 1.0F);
				...
				if (itemstack.isEmpty()) {
					((EntityPlayer) playerIn)
					.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.ZUNDOU_MIZU)));
				}
				else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.ZUNDOU_MIZU)))) {
					playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.ZUNDOU_MIZU)), false);
				}
			return true;
		}
		return true;
