package com.ayutaki.chinjufumod.init.woods.treegen;
...
public class WorldGenTreeSakuraBig extends WorldGenAbstractTree {
	...
	List<WorldGenTreeSakuraBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTreeSakuraBig(boolean notify) {
		super(notify);
		this.leave = ASDecoModWoods.SAKURA_flow.getDefaultState();
	}

	public WorldGenTreeSakuraBig(boolean notify, IBlockState state) {
		super(notify);
	  this.leave = state;
	}
	...
	public Block getLog() {
	  return ASDecoModWoods.SAKURA_log;
	}

	public Block getSapling() {
	  return ASDecoModWoods.SAKURA_nae;
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTreeSakuraBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTreeSakuraBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTreeSakuraBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeaves() {
		...
		for (WorldGenTreeSakuraBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
