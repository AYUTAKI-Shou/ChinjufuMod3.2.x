		...
		if (item != TTimeItems.Item_SAKEBOTTLE) {
			...
			if (itemstack.isEmpty()) {
				((EntityPlayer) playerIn).inventory
						.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.WINEBOT_1)));
			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.WINEBOT_1)))) {
				playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ASDecoModHakkou.WINEBOT_1)), false);
			}
