...
public abstract class KansaikiThrowable extends EntityThrowable {

	public static final DataParameter<Boolean> INITIATED = EntityDataManager
			.<Boolean>createKey(KansaikiThrowable.class, DataSerializers.BOOLEAN);

	private final String damageName;

	public KansaikiThrowable(World worldIn, EntityLivingBase thrower, ItemStack stack, String damageName) {
		super(worldIn, thrower);
		this.damageName = damageName;

		/* 初期状態での向き */
		setLocationAndAngles(thrower.posX, thrower.posY + thrower.getEyeHeight(),
	...
	private void setThrowableHeading(double motionX, double motionY, double motionZ, float f, float g) {
		// TODO 自動生成されたメソッド・スタブ

	}

	public KansaikiThrowable(World worldIn, String damageName) {
		super(worldIn);
		this.damageName = damageName;
	}
