...
public class BlockNabeNimame_boil extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side,
		...
		if (item != Items.BOWL) {
			...
			if (playerIn instanceof EntityPlayer) {
				((EntityPlayer) playerIn).inventory.addItemStackToInventory(new ItemStack(TTimeItems.NIMAME, 1));
			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(TTimeItems.NIMAME))) {
				playerIn.dropItem(new ItemStack(TTimeItems.NIMAME), false);
			}
	...
	public void addInformation(ItemStack stack, @Nullable World playerIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.tile.block_food_nabenimame_b.name", meta));
	}
