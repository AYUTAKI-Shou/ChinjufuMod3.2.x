...
public class BlockTamagoyaki_bake extends BlockFacingSapo {
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
		...
		if (random.nextDouble() < 0.1D) {
			...
			if (world.getBlockState(new BlockPos(i, j - 1, k)).getBlock() == Blocks.LIT_FURNACE || world
				...
				world.playSound(i, j, k, SoundsHandler.JUU, SoundCategory.BLOCKS, 0.2F, 1.0F, false);
			}
		}
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (item != TTimeItems.Item_SARA) {
			...
			if (itemstack.isEmpty()) {
				...
				.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.TAMAGOYAKI_1)));

			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.TAMAGOYAKI_1)))) {
				playerIn.dropItem(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.TAMAGOYAKI_1)), false);
			}
	...
	public void addInformation(ItemStack stack, @Nullable World playerIn, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("tips.tile.block_food_tgy_bake.name", meta));
	}
}
