...
public class WorldGenTreeOakKareBig extends WorldGenAbstractTree {
	...
	List<WorldGenTreeOakKareBig.FoliageCoordinates> foliageCoords;
	/* 葉っぱのフィールドを追加 */
	private IBlockState leave;

	public WorldGenTreeOakKareBig(boolean notify) {
		super(notify);
		this.leave = ASDecoModWoods.OAKKARE_leaf.getDefaultState();
	}

	public WorldGenTreeOakKareBig(boolean notify, IBlockState state) {
		super(notify);
	  this.leave = state;
	}
	...
	public Block getSapling() {
	  return ASDecoModWoods.OAKKARE_nae;
	}
	...
	void generateLeafNodeList() {
		...
		this.foliageCoords = Lists.<WorldGenTreeOakKareBig.FoliageCoordinates>newArrayList();
		this.foliageCoords.add(new WorldGenTreeOakKareBig.FoliageCoordinates(this.basePos.up(k), j));
				...
				for (int l = 0; l < i; ++l) {
					...
					if (this.checkBlockLine(blockpos, blockpos1) == -1) {
						...
						if (this.checkBlockLine(blockpos2, blockpos) == -1) {
							this.foliageCoords.add(new WorldGenTreeOakKareBig.FoliageCoordinates(blockpos, blockpos2.getY()));
						}
					}
				}
			}
	...
	void generateLeaves() {
		...
		for (WorldGenTreeOakKareBig.FoliageCoordinates BlockBigTreeSakura$foliagecoordinates : this.foliageCoords) {
