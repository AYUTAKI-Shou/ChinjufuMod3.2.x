...
public class BlockSakura_flow extends BlockLeaves {

	public static final PropertyBool DECAYABLE = PropertyBool.create("decayable");
	public static final PropertyBool CHECK_DECAY = PropertyBool.create("check_decay");

	public BlockSakura_flow() {
		super();
	...
	public java.util.List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (rand.nextInt(chance) == 0)
			stack.add(new ItemStack(ASDecoModWoods.SAKURA_nae, 1, damageDropped(state)));

		if (rand.nextInt(chance) == 0)
			stack.add(new ItemStack(ChinjufuModItemFoods.FOOD_CHERRY, 1, damageDropped(state)));
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
		...
		if (world.isAirBlock(pos.down())) {
			EntityFallSakura petal = new EntityFallSakura(world);
