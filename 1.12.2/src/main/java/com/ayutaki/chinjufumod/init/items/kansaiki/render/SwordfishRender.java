...
public abstract class SwordfishRender<T extends Entity> extends Render<T> {

	ModelSwordfish model;
	float scale = 1;
	int modifier = 0;

	protected SwordfishRender(RenderManager renderManager, ModelSwordfish model, float scale, int rotationModifier) {
		this(renderManager, model, rotationModifier);
		this.scale = scale;
	}

	protected SwordfishRender(RenderManager renderManager, ModelSwordfish model, int rotationModifier) {
		super(renderManager);
		this.model = model;
