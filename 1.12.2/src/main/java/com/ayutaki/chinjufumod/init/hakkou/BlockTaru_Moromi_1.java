...
public class BlockTaru_Moromi_1 extends Block {

	public BlockTaru_Moromi_1() {
		super(Material.WOOD);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side,
		...
		if (item != Items.BOWL) {
			...
			if (itemstack.isEmpty()) {
				((EntityPlayer) playerIn).inventory.addItemStackToInventory(new ItemStack(TTimeItems.MORO));
			}
			else if (!playerIn.inventory.addItemStackToInventory(new ItemStack(TTimeItems.MORO))) {
				playerIn.dropItem(new ItemStack(TTimeItems.MORO), false);
			}
