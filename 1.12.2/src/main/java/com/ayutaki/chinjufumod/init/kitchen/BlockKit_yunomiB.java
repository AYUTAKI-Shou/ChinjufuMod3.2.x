...
public class BlockKit_yunomiB extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side,
		...
		if (itemstack.isEmpty()) {
			((EntityPlayer) playerIn).inventory.addItemStackToInventory(new ItemStack(TTimeItems.Item_YUNOMI));
			worldIn.playSound(null, pos, SoundEvents.ENTITY_ITEM_PICKUP, SoundCategory.PLAYERS, 0.2F, 2.0F);

			return worldIn.setBlockState(pos, ASDecoModKitchen.KIT_YUNOMIA.getDefaultState()
					.withProperty(FACING, state.getValue(FACING)));
		}

		else if (item == Item.getItemFromBlock(ChinjufuModFoodBlocks.KYUSU_kara)) {

			itemstack.shrink(1);
			worldIn.playSound(null, pos, SoundEvents.BLOCK_GLASS_PLACE, SoundCategory.BLOCKS, 1.0F, 1.5F);
			return worldIn.setBlockState(pos, ASDecoModKitchen.KIT_YUNOMIC
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		List<ItemStack> stack = new ArrayList<ItemStack>();
	        ...
	        stack.add(new ItemStack(TTimeItems.Item_YUNOMI, 6, this.damageDropped(state)));
